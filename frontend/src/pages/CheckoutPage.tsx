...
// COMPONENTE DEL ABANICO DE TARJETAS (RESTAURADO)
const CardHoverStackTarjeta = ({ visibleCardsCount = 5, totalCards = 5 }) => {
  const [isHovered, setIsHovered] = useState(false);
  const cardLogos = [
    '/visa.sxIq5Dot.svg',
    '/master.CzeoQWmc.svg',
  ...
  return (
    <div className="flex items-center gap-5 relative h-14 cursor-pointer" onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-16">
              ...
              <section className="space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <button type="button" onClick={() => setPaymentMethod('mercado_pago')}
                    className={`p-10 rounded-[3rem] border-2 transition-all flex flex-col items-center gap-4 ${paymentMethod === 'mercado_pago' ? 'border-[#009EE3] bg-blue-50/40 shadow-xl shadow-blue-100' : 'border-gray-50 bg-[#F9F9F9]'}`}>
                    <img src="/mercadopago.BK20nVmQ.svg" alt="MP" className="h-10 object-contain" />
                    <span className={`text-[10px] font-black uppercase tracking-widest ${paymentMethod === 'mercado_pago' ? 'text-[#009EE3]' : 'text-gray-400'}`}>Mercado Pago</span>
                    <CardHoverStack />

                  </button>

                  <button type="button" onClick={() => setPaymentMethod('card')}
                    className={`p-10 rounded-[3rem] border-2 transition-all flex flex-col items-center gap-4 ${paymentMethod === 'card' ? 'border-black bg-gray-50' : 'border-gray-50 bg-[#F9F9F9]'}`}>
                    <CreditCard size={40} className={paymentMethod === 'card' ? 'text-black' : 'text-gray-300'} />
                    <span className={`text-[10px] font-black uppercase tracking-widest ${paymentMethod === 'card' ? 'text-black' : 'text-gray-400'}`}>Tarjeta</span>
                    <CardHoverStackTarjeta />
                  </button>
                </div>
                ...
                <AnimatePresence>
                  {paymentMethod === 'card' && (
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                      <div className="bg-[#F9F9F9] p-10 rounded-[3rem] border border-gray-100 space-y-5 mt-6 shadow-inner">
                        <input {...register('cardNumber')} placeholder="Número de tarjeta" className="input-white" />
                        <div className="grid grid-cols-2 gap-5">
                          <input {...register('expiry')} placeholder="MM/AA" className="input-white" />
                          <input {...register('cvv')} placeholder="CVV" className="input-white" />
                        </div>
                      </div>
                    </motion.div>
                  )}
              ...
              <button disabled={!isValid || isSubmitting} type="submit"
                className={`w-full py-8 rounded-full text-[11px] font-black uppercase tracking-[0.4em] transition-all shadow-2xl ${paymentMethod === 'mercado_pago' ? 'bg-[#009EE3] text-white hover:scale-[1.01]' : 'bg-black text-white hover:scale-[1.01]'}`}>
                {isSubmitting ? "Procesando..." : `Finalizar Compra $${totalPrice().toLocaleString('es-AR')}`}
              </button>
            </form>
          </div>
